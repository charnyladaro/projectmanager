...
function stopTimer(taskId) {
    if (timers[taskId]) {
        clearInterval(timers[taskId].interval);
        const totalElapsedTime = Math.floor((Date.now() - timers[taskId].startTime) / 1000 / 60); // Convert milliseconds to minutes
...
function updateTimer(taskId) {
    const elapsedTime = Math.floor((Date.now() - timers[taskId].startTime) / 1000); // Convert milliseconds to seconds
    const minutes = Math.floor(elapsedTime / 60);
    const seconds = elapsedTime % 60;
    document.getElementById(`timer-display-${taskId}`).textContent = `Timer: ${minutes}m ${seconds}s`;
}
